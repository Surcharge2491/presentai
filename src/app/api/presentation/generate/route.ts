...
interface SlidesRequest {
  ...
  modelProvider?: string; // Model provider (openai, anthropic, or lmstudio)
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error('Failed to get Supabase user:', authError);
      return NextResponse.json(
        { error: "Unauthorized - Please sign in to generate presentations" },
        { status: 401 }
      );
    }
